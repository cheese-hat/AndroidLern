...
public class Program {
    ...
    public void pseudoMain(Input in, Output out, Drawer drawer){
        ...
        String startUserPointXY = in.readLine();
        String[] strStartUserPointXY = startUserPointXY.split(" ");
        Point startUserPoint = new Point(Integer.parseInt(strStartUserPointXY[0]), Integer.parseInt(strStartUserPointXY[1]));
        drawer.drawPoint(startUserPoint.x, startUserPoint.y, 0xffff0000);
        drawer.drawText("*" ,startUserPoint.x, startUserPoint.y);
        ...
        for (int k = 0; k < quantityPoints; k++) {
            for (int j = 0; j < quantityPoints - 1; j++){
                ...
                double distPoint1 = distancePoints(startUserPoint, point1);
                double distPoint2 = distancePoints(startUserPoint, point2);
    ...
    public static double distancePoints(Point startUserPoint, Point userPoint){
        double dXsquare = (double) Math.pow((userPoint.x - startUserPoint.x), 2);
        double dYsquare = (double) Math.pow((userPoint.y - startUserPoint.y), 2);
